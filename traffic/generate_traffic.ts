...
async function main() {
  ...
  await runPhase({ baseUrl, phaseName: 'warmup', scenario: 'normal', durationMs: 30_000, rps: 1 });
  await runPhase({ baseUrl, phaseName: 'latency', scenario: 'latency', durationMs: 90_000, rps: 2 });
  await runPhase({ baseUrl, phaseName: 'error', scenario: 'error', durationMs: 90_000, rps: 2 });
  await runPhase({ baseUrl, phaseName: 'security', scenario: 'security', durationMs: 60_000, rps: 2 });
